<?php
// checkout.php - Complete checkout process from scratch
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    $valid_payment_methods = ['credit_card', 'paypal', 'razorpay'];
    ...
    if (empty($errors)) {
        try {
            ...
            foreach ($cart_items as $product_id => $item) {
                ...
                $check_products_table = $conn->query("SHOW TABLES LIKE 'products'");
                if ($check_products_table->num_rows > 0) {
            ...
            $payment_result = $secure_payment_processor->processSecurePayment($order_data, $payment_method, $customer_data);
            ...
            if ($payment_result['success']) {
                ...
                // Redirect to order confirmation or payment page based on method
                if ($payment_method === 'razorpay') {
                    // For Razorpay, we need to redirect to a payment page
                    ...
                    header("Location: /bookshelf/payment.php?method=razorpay&order=" . urlencode($order_number));
                } else {
                    // For other methods, go to confirmation
                ...
                $conn->rollback();
                $error_message = "Payment processing failed: " . ($payment_result['error'] ?? 'Unknown error');
            }
